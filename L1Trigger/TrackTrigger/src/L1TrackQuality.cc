...
float L1TrackQuality::runEmulatedTQ(std::vector<float> inputFeatures) {
    // load in bdt
    conifer::BDT<float,float> bdt(this->ONNXmodel_.fullPath());

    // collect features and classify using bdt
    std::vector<float> output = bdt.decision_function(inputFeatures);
