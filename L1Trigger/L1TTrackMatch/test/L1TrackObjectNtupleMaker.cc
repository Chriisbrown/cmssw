...
private:
  ...
  edm::InputTag TrackJetsEmulationInputTag;
  ...
  edm::InputTag TrackJetsExtendedEmulationInputTag;
  ...
  edm::EDGetTokenT<l1t::VertexWordCollection> L1VertexWordToken_;
  ...
  edm::EDGetTokenT<l1t::TkJetWordCollection> TrackJetsEmulationToken_;	
  edm::EDGetTokenT<l1t::TkJetWordCollection> TrackJetsExtendedEmulationToken_;
  ...
  std::vector<float>* m_trkExt_MVA1;
  ...
  std::vector<float>* m_matchtrkExt_MVA1;
...
L1TrackObjectNtupleMaker::L1TrackObjectNtupleMaker(edm::ParameterSet const& iConfig) : config(iConfig) {
  ...
  SaveTrackJetsEmulation = iConfig.getParameter<bool>("SaveTrackJetsEmulation");
  SaveTrackMET = iConfig.getParameter<bool>("SaveTrackMET");

  SaveVertexEmulation = iConfig.getParameter<bool>("SaveVertexEmulation");
  ...
  if (Displaced == "Prompt" || Displaced == "Both") {
    ...
    TrackJetsEmulationInputTag = iConfig.getParameter<InputTag>("TrackJetsEmulationInputTag");
    ...
    TrackJetsEmulationToken_ = consumes<l1t::TkJetWordCollection>(TrackJetsEmulationInputTag);
  ...
  if (Displaced == "Displaced" || Displaced == "Both") {
    ...
    TrackJetsExtendedEmulationInputTag = iConfig.getParameter<InputTag>("TrackJetsExtendedEmulationInputTag");
    ...
    TrackJetsExtendedEmulationToken_ = consumes<l1t::TkJetWordCollection>(TrackJetsExtendedEmulationInputTag);
  ...
  if (SaveVertexEmulation)
    L1VertexWordToken_ = consumes<l1t::VertexWordCollection>(RecoVertexInputTag);
  else
...
void L1TrackObjectNtupleMaker::beginJob() {
  ...
  m_trkExt_MVA1 = new std::vector<float>;
  ...
  m_matchtrkExt_MVA1 = new std::vector<float>;
...
void L1TrackObjectNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  //Vertex
  edm::Handle<l1t::VertexWordCollection> L1TkPrimaryVertexWordHandle;
  edm::Handle<l1t::VertexCollection> L1TkPrimaryVertexHandle;

  if (SaveVertexEmulation)
    iEvent.getByToken(L1VertexWordToken_, L1TkPrimaryVertexWordHandle);
  else
    iEvent.getByToken(L1VertexToken_, L1TkPrimaryVertexHandle);
  std::vector<l1t::Vertex>::const_iterator vtxIter;
  std::vector<l1t::VertexWord>::const_iterator vtxWordIter;
  ...
  if (Displaced == "Prompt" || Displaced == "Both") {
    ...
    iEvent.getByToken(TrackJetsEmulationToken_,TrackJetsEmulationHandle);
  ...
  if (Displaced == "Displaced" || Displaced == "Both") {
    ...
    iEvent.getByToken(TrackJetsExtendedEmulationToken_,TrackJetsExtendedEmulationHandle);
  ...
  if (SaveAllTracks && (Displaced == "Displaced" || Displaced == "Both")) {
    ...
    for (iterL1Track = TTTrackExtendedHandle->begin(); iterL1Track != TTTrackExtendedHandle->end(); iterL1Track++) {
      ...
      m_trkExt_MVA1->push_back(tmp_trk_MVA1);
  ...
  for (iterTP = TrackingParticleHandle->begin(); iterTP != TrackingParticleHandle->end(); ++iterTP) {
    ...
    if (Displaced == "Displaced" || Displaced == "Both") {
      ...
      float tmp_matchtrkExt_MVA1 = -999;
      ...
      if (nMatch > 0) {
        ...
        tmp_matchtrkExt_MVA1 = matchedTracks.at(i_track)->trkMVA1();
      ...
      m_matchtrkExt_MVA1->push_back(tmp_matchtrkExt_MVA1);
  ...
  if (SaveVertexEmulation){
    if (L1TkPrimaryVertexWordHandle.isValid()) {
        for (vtxWordIter = L1TkPrimaryVertexWordHandle->begin(); vtxWordIter != L1TkPrimaryVertexWordHandle->end(); ++vtxWordIter) {
          m_pv_L1reco->push_back(vtxWordIter->z0());
          m_pv_L1reco_sum->push_back(vtxWordIter->pt());
          std::cout << vtxWordIter->z0() << std::endl;
        }
      }
      else {
        edm::LogWarning("DataNotFound")<< "\nWarning: L1TkPrimaryVertexWordHandle not found in the event"<< std::endl;
      }
    }
  else{
